...
class RoomMappingService {
    ...
    getRoomId(frontendIndex, floor, roomType) {
        try {
            if (!this.initialized) {
                log.warn("Room mapping service not initialized, using fallback calculation");
            ...
            if (!roomData) {
                log.warn(`Room index ${frontendIndex} not found in floor ${floor}, using fallback`);
                return this.getFallbackRoomId(frontendIndex, floor);
            }

            // Validate room type matches expected type
            if (roomData.type !== roomType) {
                log.warn(`Room type mismatch: expected ${roomType}, got ${roomData.type}`);
            }

            log.debug(`Room mapping: Floor ${floor}, Index ${frontendIndex} (${roomType}) -> ID ${roomData.id}`);
            return roomData.id;
    ...
    getFallbackRoomId(frontendIndex, floor) {
        ...
        log.debug(`Fallback room ID calculation: Floor ${floor}, Index ${frontendIndex} -> ID ${roomId}`);
        return roomId;
    }
