...
class EnemyMappingService {
    ...
    async initialize() {
        try {
            ...
            this.debugInfo = {
                ...
                supportedTypes: Object.keys(this.enemyTypeMap).filter(key => !key.includes('_'))
            };
    ...
    getEnemyId(enemyTypeName) {
        try {
            ...
            if (!this.initialized) {
                log.warn("Enemy mapping service not initialized, using fallback calculation");
            ...
            if (enemyId) {
                ...
                log.warn(`Enemy type "${normalizedName}" not found in mapping, using fallback`);
                return this.getFallbackEnemyId(normalizedName);
            }
    ...
    getEnemyData(enemyId) {
        try {
            ...
            for (const [typeName, id] of Object.entries(this.enemyTypeMap)) {
                if (id === enemyId && !typeName.includes('_')) { // Skip snake_case aliases
                    return {
                        id: enemyId,
                        typeName: typeName,
