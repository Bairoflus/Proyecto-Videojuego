...
export class EnhancedErrorHandler {
  ...
  showOfflineMode() {
    if (window.enhancedLoadingSystem) {
      window.enhancedLoadingSystem.showError({
        ...
        details:
          "Some features may be limited. Changes will be saved when connection is restored.",
        type: "warning",
        persistent: true,
        autoHide: false,
      });
  ...
  showConnectionRestored() {
    if (window.enhancedLoadingSystem) {
      window.enhancedLoadingSystem.showSuccess(
  ...
  getErrorMetrics() {
    ...
    return {
      ...this.errorMetrics,
      recoveryRate:
        total > 0 ? (this.errorMetrics.recoveredErrors / total) * 100 : 0,
...
export const apiRequestEnhanced = (
  endpoint,
  options = {},
  retryConfig = {}
) => {
  return enhancedErrorHandler.apiRequestWithRetry(
    endpoint,
    options,
    retryConfig
  );
