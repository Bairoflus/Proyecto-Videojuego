...
export class FloorGenerator {
  constructor() {
    ...
    this.runCount = parseInt(
      localStorage.getItem("gameRunCount") ||
        FLOOR_CONSTANTS.INITIAL_RUN_COUNT.toString(),
      10
    );
  ...
  getCurrentRoom() {
    ...
    if (isCombatRoom) {
      log.info(
        `Created new combat room ${roomIndex} with ${room.objects.enemies.length} enemies`
      );
    } else {
  ...
  nextFloor() {
    if (this.floorCount >= FLOOR_CONSTANTS.MAX_FLOORS_PER_RUN) {
      ...
      this.floorCount = 1;
      log.info(
        `Completed floor ${FLOOR_CONSTANTS.MAX_FLOORS_PER_RUN}, starting new run:`,
        this.runCount
      );
    } else {
